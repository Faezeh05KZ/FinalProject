...
void GameManager::initializeCards() {
    for(int i = 0; i < 3; ++i) 
        perkCardDeck.push_back(make_shared<PerkCard>("Visit from the Detective", PerkCard::PerkEffectType::VISIT_FROM_THE_DETECTIVE, "Place Invisible Man in any location"));
    for(int i = 0; i < 3; ++i) 
        perkCardDeck.push_back(make_shared<PerkCard>("Break of Dawn", PerkCard::PerkEffectType::BREAK_OF_DAWN, "Skip the next monster phase. Draw 2 items."));
    for(int i = 0; i < 4; ++i) 
        perkCardDeck.push_back(make_shared<PerkCard>("Overstock", PerkCard::PerkEffectType::OVERSTOCK, "Each player draws 1 item."));
    for(int i = 0; i < 4; ++i) 
        perkCardDeck.push_back(make_shared<PerkCard>("Late into the Night", PerkCard::PerkEffectType::LATE_INTO_THE_NIGHT, "Current player gets 2 extra actions."));
    for(int i = 0; i < 3; ++i) 
        perkCardDeck.push_back(make_shared<PerkCard>("Repel", PerkCard::PerkEffectType::REPEL, "Move each monster up to 2 spaces."));
    for(int i = 0; i < 3; ++i) 
        perkCardDeck.push_back(make_shared<PerkCard>("Hurry", PerkCard::PerkEffectType::HURRY, "Move each hero up to 2 spaces."));

    shuffle(perkCardDeck.begin(), perkCardDeck.end(), default_random_engine(random_device()()));

    for(int i = 0; i < 3; ++i) 
        monsterCardDeck.push_back(make_shared<MonsterCard>("Form of The Bat", 2, vector<MonsterStrike>{{"Invisibleman", 1, 2}} ));
    for(int i = 0; i < 3; ++i) 
        monsterCardDeck.push_back(make_shared<MonsterCard>("Sunrise", 0, vector<MonsterStrike>{{"Invisibleman", 1, 2}, {"Frenzied", 0, 0}} ));
    for(int i = 0; i < 5; ++i) 
        monsterCardDeck.push_back(make_shared<MonsterCard>("Thief", 2, vector<MonsterStrike>{{"Invisibleman", 1, 3}, {"Dracula", 0, 0}} ));
    for(int i = 0; i < 2; ++i) 
        monsterCardDeck.push_back(make_shared<MonsterCard>("Hypnotic Gaze", 2, vector<MonsterStrike>{{"Invisibleman", 1, 2}} ));
    for(int i = 0; i < 2; ++i) 
    monsterCardDeck.push_back(make_shared<MonsterCard>("On The Move", 3, vector<MonsterStrike>{{"Frenzied", 3, 2}} ));
...
void GameManager::startGame(const vector<string>& playerNames) {
    ...
    auto getGarlicTime = [](const string& playerName) {
        int time;
        while (true) {
            cout << playerName << ", how many hours since you last ate garlic? ";
            cin >> time;
            
            if (cin.fail() || time < 0) {
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                cout << "Invalid input! Please enter a positive integer number." << endl;
            } else {
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                return time;
            }
    ...
    int time1 = getGarlicTime(playerNames[0]);
    int time2 = getGarlicTime(playerNames[1]);
...
void GameManager::startNextTurn() {
    ...
    getCurrentHero()->resetActions(); // بازنشانی اقدامات قهرمان
}
...
bool GameManager::moveHero(const string& heroName, const string& destination) {
    ...
    if (gameMap->areConnected(hero->getCurrentLocation(), destination)) {
        string currentLocation = hero->getCurrentLocation();
        // انجام حرکت
        hero->setCurrentLocation(destination);
        cout << hero->getName() << " moved to " << destination << "." << endl;
        // حرکت دادن روستایی همراه 
        auto villagersAtLocation = gameMap->getVillagersAt(currentLocation);
        for (const auto& villager : villagersAtLocation) {
            if (villager->isInPlay()) {
                villager->moveTo(destination);
                cout << villager->getName() << " was guided by " << heroName << " to " << destination << "." << endl;
                // بررسی اینکه ایا روستایی به مکان امن رسیده یا نه 
                if (destination == villager->getSafeLocation()) {
                    rescueVillager(villager->getName(), heroName);
                }
            }
        }
...
bool GameManager::guideVillager(const string& heroName, const string& villagerName, const string& destination) {
    ...
    if (!hero || !villager || !villager->isInPlay()) 
        return false;
...
bool GameManager::pickUpItems(const string& heroName, const vector<string>& itemNames) {
    auto hero = getHeroByName(heroName);
    if (!hero) 
        return false;
    
    bool anyItemPicked = false;        
    // حذف ایتم از نقشه و اضافه کردن ان به موجودی قهرمان
...
bool GameManager::useSpecialAbility(const string& heroName, const string& optionalTarget) {
    ...
    if (auto archaeologist = dynamic_pointer_cast<Archaeologist>(hero)) {
        ...
        // قدرت خاص باستان شناس : برداشتن ایتم از مکان های مجاور
...
bool GameManager::usePerkCard(const string& heroName, const string& cardName) {
    ...
    auto it = find_if(hero->getPerkCards().begin(), hero->getPerkCards().end(), [&](const shared_ptr<PerkCard>& card){ 
            return GameUtils::toLower(card->getCardName()) == GameUtils::toLower(cardName); });
...
bool GameManager::advanceTask(const string& heroName, const string& monsterName) {
    ...
    // وظیفه مربوط به مرد نامرئی: جمع‌آوری شواهد
...
bool GameManager::defeatMonster(const string& heroName, const string& monsterName) {
    ...
    // شکست مرد نامرئی با آیتم‌های قرمز
    else if (auto invisibleMan = dynamic_pointer_cast<InvisibleMan>(monster)) {
...
void GameManager::resolveStrikes(const vector<MonsterStrike>& strikes) {
    ...
    for (const auto& strike : strikes) {
        ...
        if (strike.targetMonster == "Frenzied") monsterToAct = getMonsterByName(frenziedMonster);
            ...
            for (auto& m : monsters) {
                if (!m->isDefeated() && !attackMadeThisTurn) {
                    ...
                    if(!m->getTargetsAtLocation(*this).empty()) { 
                        handleAttack(m, strike.dice); attackMadeThisTurn = true;
                    }
                }
            }
            continue;
        ...
        if (monsterToAct && !monsterToAct->isDefeated()) {
            ...
            if (!monsterToAct->getTargetsAtLocation(*this).empty()) { 
                handleAttack(monsterToAct, strike.dice); attackMadeThisTurn = true; 
            }
        }
    }
}
...
void GameManager::moveMonsterTowardsTarget(const shared_ptr<Monster>& monster, int steps) {
    ...
    if (target.distance != numeric_limits<int>::max()) {
        ...
        for (int i = 0; i < steps; ++i) {
            ...
            if (nextStep == current) 
                break; // اگر مسیری وجود نداشت یا به مقصد رسیدیم، متوقف شو
            current = nextStep;
            if (current == target.location) 
                break; // اگر به مقصد رسیدیم، متوقف شو
        }
...
void GameManager::handleAttack(const shared_ptr<Monster>& monster, int diceCount) {
    // اگر تعداد تاس ها صفر باشد حمله انجام نمیشه
    if (diceCount == 0) 
        return;
    // اگر هدف وجود نداشت حمله انجام نمیشه
    ...
    for (int i = 0; i < diceCount; ++i) {
        ...
        if (roll == 1)      
            powerActivated = true;  // نماد !
        else if (roll == 2) 
            attackHit = true;       // نماد *
    }
    // فعال شدن قدرن ویژه هیولا
    if (powerActivated) {
    ...
    if (attackHit) {
        ...
        sort(targets.begin(), targets.end(), [](const auto& a, const auto& b) { return a.type < b.type; }); // اولویت بندی هدف حمله
...
bool GameManager::defendAttack(const shared_ptr<Hero>& hero) {
    // اگر ایتم نداشته باشد نی توانند دفاع کند 
    if (hero->getItems().empty()) 
        return false;
...
void GameManager::drawAndPlaceItems(int count) {
    for (int i = 0; i < count; ++i) {
        if (itemPool.empty()) { 
            cout << "Item pool is empty!" << endl; 
            return; 
        }
        // انتخاب تصادفی ایتم و قرار دادن در جای خودش
...
void GameManager::thiefEvent() {
    ...
    if (!locWithMostItems.empty()) {
        ...
        // حذف همه آیتم‌های مکان
        auto items = gameMap->getItemsAt(locWithMostItems);
        for(const auto& item : items) 
            gameMap->removeItem(locWithMostItems, item->getName());
    }
}
...
void GameManager::updateFrenzyOrder(bool forceNext) {
    ...
    if (nextMonster && !nextMonster->isDefeated()) 
        frenziedMonster = nextFrenziedName;
    else {
...
std::string GameManager::getCurrentPlayerName() const { 
    return players.at(currentPlayerIndex)->getName(); 
}

shared_ptr<Hero> GameManager::getCurrentHero() const { 
    if (players.empty()) 
        return nullptr; 
    return players.at(currentPlayerIndex)->getHero(); 
}

shared_ptr<Hero> GameManager::getHeroByName(const string& name) const {
...
shared_ptr<Monster> GameManager::getMonsterByName(const string& name) const { 
...
shared_ptr<Villager> GameManager::getVillagerByName(const string& name) const { 
    for (const auto& v : villagers) {
        if (v->getName() == name) 
            return v;   
    }
...
void GameManager::promptAndPlaceMonster(const std::string& monsterName) {
    auto monster = getMonsterByName(monsterName);
    if (!monster) {
        cout << "Monster " << monsterName << " not found." << endl;
        return;
    }
    auto validLocations = gameMap->getAllLocations();
    cout << "Valid locations for " << monsterName << ":" << endl;
    for (size_t i = 0; i < validLocations.size(); ++i) {
        cout << i + 1 << ". " << validLocations[i] << endl;
    }
    cout << "> ";
    int choice;
    cin >> choice;
    cin.ignore(numeric_limits<streamsize>::max(), '\n');
    if (choice >= 1 && choice <= static_cast<int>(validLocations.size())) {
        placeMonster(monsterName, validLocations[choice - 1]);
    } else {
        cout << "Invalid location. Placement cancelled." << endl;
    }
}

void GameManager::promptAndMoveMonster(const std::string& monsterName, int steps) {
    auto monster = getMonsterByName(monsterName);
    if (!monster || monster->isDefeated()) {
        cout << monsterName << " cannot be moved." << endl;
        return;
    }
    string currentLoc = monster->getCurrentLocation();
    for (int i = 0; i < steps; ++i) {
        auto connected = gameMap->getConnectedLocations(currentLoc);
        if (connected.empty()) {
            cout << "No valid moves available for " << monsterName << "." << endl;
            return;
        }
        cout << "Choose a destination for " << monsterName << " (step " << i + 1 << "/" << steps << "):" << endl;
        for (size_t j = 0; j < connected.size(); ++j) {
            cout << j + 1 << ". " << connected[j] << endl;
        }
        cout << "> ";
        int choice;
        cin >> choice;
        ...
        if (choice >= 1 && choice <= static_cast<int>(connected.size())) {
            currentLoc = connected[choice - 1];
            monster->setCurrentLocation(currentLoc);
            cout << monsterName << " moved to " << currentLoc << "." << endl;
        } else {
            cout << "Invalid choice. Move cancelled." << endl;
            break;
        }
    }
}

void GameManager::promptAndMoveHero(const std::string& heroName, int steps) {
    auto hero = getHeroByName(heroName);
    if (!hero || hero->isDamaged()) {
        cout << heroName << " cannot be moved." << endl;
        return;
    }
    string currentLoc = hero->getCurrentLocation();
    for (int i = 0; i < steps; ++i) {
        auto connected = gameMap->getConnectedLocations(currentLoc);
        if (connected.empty()) {
            cout << "No valid moves available for " << heroName << "." << endl;
            return;
        }
        cout << "Choose a destination for " << heroName << " (step " << i + 1 << "/" << steps << "):" << endl;
        for (size_t j = 0; j < connected.size(); ++j) {
            cout << j + 1 << ". " << connected[j] << endl;
        }
        cout << "> ";
        ...
        if (choice >= 1 && choice <= static_cast<int>(connected.size())) {
            currentLoc = connected[choice - 1];
            hero->setCurrentLocation(currentLoc);
            cout << heroName << " moved to " << currentLoc << "." << endl;
        } else {
            cout << "Invalid choice. Move cancelled." << endl;
            break;
        }
